...
private:
	...
	void publishCmdCallBack(const ros::TimerEvent& e){
		...
		if(motors_on){
			//take off mode
                        //double alt_cmd = (min(1.0, max(-1.0, double(recent_joy.axes[3]))) + 1.0) / 2.0;
			if(take_off_mode and not took_off){
                                 ROS_INFO("debug5");
				 nh.setParam("/fcu/position_control","off");
				nh.setParam("/fcu/fcu/position_control","off");
				nh.setParam("/fcu/fcu/state_estimation", "off");
				ctrl_msg_out.type = asctec_hl_comm::mav_ctrl::acceleration;
				if(recent_z < 0.40){
					v_tx = 0.0;
					v_ty = 0.0;
					v_tz = 0.25*cos(M_PI*recent_z) + 0.3;
					v_tyaw = 0.0;
				}
			...
			//manual mode
				...
				if(land_mode){					
					...
					double alt_cmd = (min(1.0, max(-1.0, double(v_tz)) + 1.0) / 2.0);
                                        v_tz = alt_cmd;
