  ...
  void joyCallBack(const sensor_msgs::JoyConstPtr& joyMsg)
   {
	  ...
	  //ESTOP
 	  ...
 	  //turn on the motors
 	  if (not motors_on and (joyMsg->buttons[1]) and (joyMsg->axes[3] <= -0.98)) // button 2 and full min altitude to start motors 
 	          {
   ...
   void publishCmdCallBack(const ros::TimerEvent& e){
	 ...
	 if(motors_on){
		 ...
		 if(recent_joy.buttons[0] and !motors_on){
			 nh.setParam("/fcu/position_control","GPS");
                         nh.setParam("/fcu/state_estimation","HighLevel_SSDK"); //?

			 ctrl_msg.type = asctec_hl_comm::mav_ctrl::velocity;
			 ctrl_msg.x = 0;
			 ctrl_msg.y = 0;
			 ctrl_msg.z = 0.5;
			 ctrl_msg.yaw = 0;
			 ctrl_msg.v_max_xy = 0.5; // use max velocity from config
			 ctrl_msg.v_max_z = 0.5;
                         ROS_INFO("I'm here");
 	  }
             //land 
              else if(recent_joy.buttons[0] and motors_on){
			 nh.setParam("/fcu/position_control","GPS");
                         nh.setParam("/fcu/state_estimation","HighLevel_SSDK");
 	  ...
 	  ctrl_msg.y= roll_sign * roll_in > roll_deadband ? -(std::max(-1.0, std::min(1.0, roll_in)) - roll_sign
 	          * roll_deadband) / (1.0 - roll_deadband)/al_xy : 0.0;
 	  ctrl_msg.x = pitch_sign * pitch_in > pitch_deadband ? (max(-1.0, min(1.0, pitch_in)) - pitch_sign
 	          * pitch_deadband) / (1.0 - pitch_deadband)/al_xy : 0.0;
